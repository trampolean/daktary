...
class GithubUrl {
  constructor(url) {
    ...
    if (this.isValid) {
      const [owner, repo, , branch, ...filename] = this.url.split('/')
      this.ghData = {
        keys: {
  ...
  checkGhBlob() {
    const owner = '[A-Za-z\u00C0-\u017F+\-\_]*'
    const repo = '[A-Za-z\u00C0-\u017F+\-\_]*'
  ...
  toGhApiSearch(query) {
    const {keys, owner, repo, filename} = this.ghData
    return `https://api.github.com/search/code` +
           `?q=${query}+language:Markdown+repo:${owner}/${repo}/${filename}`
  }
  toGhApiUrl() {
    const {keys, owner, repo, branch, filename} = this.ghData
